    ...
    <div class="row g-3">
        <div class="col-lg-6">
            <div class="kpi p-3">
                ...
                <div class="d-flex justify-content-between"><span class="text-muted">Status</span><span
                        class="badge-pill badge-a">{{ $reservation->status?->type ?? '-' }}</span></div>
                <hr>
                <div class="d-flex justify-content-between"><span
                        class="text-muted">Start</span><span>{{ $reservation->start_date }}</span></div>
                <div class="d-flex justify-content-between"><span
                        class="text-muted">End</span><span>{{ $reservation->end_date }}</span></div>
                <div class="d-flex justify-content-between"><span class="text-muted">Duration Per
                        Month</span><span>{{ $reservation->duration }}</span></div>
                <div class="d-flex justify-content-between"><span
                        class="text-muted">Created</span><span>{{ $reservation->created_at?->format('Y-m-d H:i') }}</span>
                </div>
            </div>
        </div>
                ...
                <div class="row g-2 mt-3">
                    <div class="col-6">
                        ...
                        @if ($reservation->user?->profileImage?->url)
                            <img class="img-fluid rounded"
                                src="{{ asset('storage/' . $reservation->user->profileImage->url) }}" alt="profile">
                        @else
                    ...
                    <div class="col-6">
                        ...
                        @if ($reservation->user?->idImage?->url)
                            <img class="img-fluid rounded" src="{{ asset('storage/' . $reservation->user->idImage->url) }}"
                                alt="id">
                        @else
                    ...
                    <div>
                        ...
                        @php
                            ...
                            $govName = is_array($gov->name ?? null)
                                ...
                                : $gov->name ?? '';
                        @endphp
                        <div class="text-muted small">{{ $govName }} {{ $govName && $cityName ? '-' : '' }}
                            {{ $cityName }}</div>
                    </div>
                    <div class="text-muted small">
                        Owner: <strong>{{ $reservation->house?->user?->first_name }}
                            {{ $reservation->house?->user?->last_name }}</strong>
