    ...
    <div class="kpi reveal delay-2 mb-3 filter-bar">
        <form class="row g-2 align-items-end" method="GET" action="{{ route('admin.houses.index') }}">
            ...
            <div class="col-lg-2">
                ...
                <select class="form-select" name="governorate_id">
                    ...
                    @foreach ($governorates as $g)
                        @php
                            $gn = is_array($g->name ?? null)
                                ? $g->name['en'] ?? ($g->name['ar'] ?? reset($g->name))
                                : $g->name ?? '';
                        @endphp
            ...
            <div class="col-lg-2">
                ...
                <input type="number" class="form-control" name="min_rooms" value="{{ request('min_rooms') }}"
                    placeholder="0">
            </div>

            <div class="col-lg-2">
                <label class="form-label small text-muted mb-1">Max Rooms</label>
                <input type="number" class="form-control" name="max_rooms" value="{{ request('max_rooms') }}"
                    placeholder="0">
            </div>
            ...
            <div class="house-card">
                ...
                <div class="house-card-body">
                    ...
                    <div class="house-owner mt-3">
                        ...
                        <div class="fw-bold">{{ optional($h->user)->first_name }} {{ optional($h->user)->last_name }}
                        </div>
                    ...
                    <div class="house-actions mt-3">
                        ...
                        <form method="POST" action="{{ route('admin.houses.status', $h) }}"
                            class="d-flex gap-2 flex-grow-1">
                            @csrf
                            @method('PATCH')
                            <select class="form-select" name="status_id">
                                @foreach ($statuses as $st)
                                    <option value="{{ $st->id }}" @selected($h->status_id == $st->id)>{{ $st->type }}
                                    </option>
                                @endforeach
                            </select>
