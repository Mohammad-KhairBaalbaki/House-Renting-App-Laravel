        ...
        <form method="POST" action="{{ route('admin.houses.city', $house) }}" class="mt-2">
          ...
          <div class="row g-3">
            <div class="col-md-12">
              ...
              <select name="city_id" class="form-select" style="border-radius:14px;padding:12px;" required>
                ...
                @foreach($cities as $c)
                  @php
                    $cName = is_array($c->name ?? null)
                      ...
                      : ($c->name ?? '');
                  @endphp
                  <option value="{{ $c->id }}" {{ (int)$currentCityId === (int)$c->id ? 'selected' : '' }}>
                    {{ $cName }}
                  </option>
                @endforeach
              </select>
        ...
        <div class="addr-card">
          ...
          @if($lat && $lng)
            <div class="small text-muted mt-2">
              Using coordinates: <strong>{{ $lat }}</strong>, <strong>{{ $lng }}</strong>
            </div>
          @else
            <div class="small text-muted mt-2">
